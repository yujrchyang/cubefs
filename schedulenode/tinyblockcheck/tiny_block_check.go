...
type TinyBlockCheckWorker struct {
	worker.BaseWorker
	sync.RWMutex
	port               string
...
func NewTinyBLockCheckWorker() *TinyBlockCheckWorker {
	return &TinyBlockCheckWorker{}
}

func (w *TinyBlockCheckWorker) Start(cfg *config.Config) (err error) {
	return w.Control.Start(w, cfg, doStart)
}

func doStart(s common.Server, cfg *config.Config) (err error) {
	w, ok := s.(*TinyBlockCheckWorker)
	if !ok {
	...
	if err = w.RegisterWorker(proto.WorkerTypeTinyBlockCheck, w.ConsumeTask); err != nil {
...
func (w *TinyBlockCheckWorker) Shutdown() {
	w.Control.Shutdown(w, doShutdown)
}

func doShutdown(s common.Server) {
	m, ok := s.(*TinyBlockCheckWorker)
	if !ok {
		return
	}
	close(m.StopC)
...
func (w *TinyBlockCheckWorker) Sync() {
	w.Control.Sync()
}

func (w *TinyBlockCheckWorker) parseConfig(cfg *config.Config) (err error) {
...
func (w *TinyBlockCheckWorker) ConsumeTask(task *proto.Task) (restore bool, err error) {
	...
	t := NewTinyBlockCheckTask(task, masterClient, true, true, DefaultSafeCleanInterval, exportDir)
	t.RunTinyExtentsCheck()
	return
}

func (w *TinyBlockCheckWorker) clustersID() []string {
	w.RLock()
	defer w.RUnlock()
...
func (w *TinyBlockCheckWorker) updateNotifyMembers() {
