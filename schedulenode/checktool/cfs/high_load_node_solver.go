...
func loadDockerIPList() (err error) {
	dockerIpMap = make(map[string]string, 0)
	buf := make([]byte, 2048)
	fd, _ := os.OpenFile("docker_ip", os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
	defer fd.Close()
	o := bufio.NewReader(fd)
	for {
