...
type Worker struct {
	...
	controlConfig         *migcore.ControlConfig
...
func (w *Worker) ConsumeCompactTask(task *proto.Task) (restore bool, err error) {
	metrics := exporter.NewModuleTP(fmt.Sprintf("%v_%v", migcore.Compact, migcore.ConsumeTask))
...
func (w *Worker) ConsumeFileMigTask(task *proto.Task) (restore bool, err error) {
	metrics := exporter.NewModuleTP(migcore.UmpKeySuffix(migcore.FileMig, migcore.ConsumeTask))
	defer metrics.Set(err)
...
func (w *Worker) execMigrationTask(task *proto.Task) (isFinished bool, err error) {
	...
	migTask := NewMigTask(localIp, task, masterClient, volumeInfo)
...
func (w *Worker) checkVolumeInfo(cluster, volume string, clientType data.ExtentClientType) (err error) {
	...
	var (
		clusterInfo *migcore.ClusterInfo
		volumeInfo  *migcore.VolumeInfo
		ok          bool
	)
	...
	if volumeInfo, err = migcore.NewVolumeInfo(cluster, volume, clusterInfo.MasterClient.Nodes(), w.controlConfig, clientType,
...
func (w *Worker) getVolumeInfo(clusterName, volName string) (volInfo *migcore.VolumeInfo, ok bool) {
	var value interface{}
	...
	var vol interface{}
	if vol, ok = value.(*migcore.ClusterInfo).GetVolByName(volName); ok {
		volInfo = vol.(*migcore.VolumeInfo)
	}
	return
}
...
func (w *Worker) parseMasterAddr(cfg *config.Config) {
	...
	for clusterName, nodes := range w.masterAddr {
		...
		w.clusterMap.Store(clusterName, migcore.NewClusterInfo(clusterName, materClient))
	}
}

func (w *Worker) getClusterInfo(cluster string) *migcore.ClusterInfo {
	value, ok := w.clusterMap.Load(cluster)
	if !ok {
		return nil
	}
	return value.(*migcore.ClusterInfo)
}

func (w *Worker) addVolume(cluster, volName string, vol *migcore.VolumeInfo) (err error) {
	defer func() {
		if err != nil {
...
func (w *Worker) workerViewInfo(res http.ResponseWriter, r *http.Request) {
	...
	migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeSuccess, Msg: "Success", Data: view})
}
...
func (w *Worker) info(res http.ResponseWriter, r *http.Request) {
	if err := r.ParseForm(); err != nil {
		...
		migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeInternalError,
			Msg: err.Error()})
		return
	}
	clusterName := r.FormValue(migcore.ClusterKey)
	volumeName := r.FormValue(migcore.VolNameKey)
	volInfo := w.volInfo(clusterName, volumeName)
	var migView *proto.VolumeDataMigView
	if volInfo != nil {
		migView = volInfo.GetVolumeView()
	}
	migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeSuccess, Msg: "Success", Data: migView})
}

func (w *Worker) volInfo(clusterName, volumeName string) (volInfo *migcore.VolumeInfo) {
	value, ok := w.clusterMap.Load(clusterName)
	if !ok {
		return
	}
...
func (w *Worker) stop(res http.ResponseWriter, r *http.Request) {
	if err := r.ParseForm(); err != nil {
		err = fmt.Errorf("parse form fail: %v", err)
		migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeParamError, Msg: err.Error()})
		return
	}
	cluster := r.FormValue(migcore.ClusterKey)
	volume := r.FormValue(migcore.VolNameKey)
	var msg string
	if vol, ok := w.getVolumeInfo(cluster, volume); ok {
		vol.State = migcore.VolClosing
		msg = "Success"
	} else {
		msg = fmt.Sprintf("cluster(%v) volume(%v) doesn't exist", cluster, volume)
	}
	migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeSuccess, Msg: msg})
}
...
func (w *Worker) setLimit(res http.ResponseWriter, r *http.Request) {
	...
	setType := r.FormValue(migcore.TypeKey)
	limit := r.FormValue(migcore.LimitSizeKey)
	var limitNum int64
	var err error
	if limitNum, err = strconv.ParseInt(limit, 10, 32); err != nil {
		msg := fmt.Sprintf("limit(%v) should be numeric", limit)
		migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeParamError, Msg: msg})
		return
	}
	if limitNum <= 0 {
	...
	switch setType {
	case migcore.InodeIdKey:
		w.inodeMigParallelNum = int32(limitNum)
	case migcore.MpIdKey:
		w.limiter.Reset(int32(limitNum))
	default:
		migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeParamError, Msg: "type invalid"})
		return
	}
	migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeSuccess, Msg: "Success"})
}
...
func (w *Worker) getLimit(res http.ResponseWriter, r *http.Request) {
	...
	migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeSuccess, Msg: "Success", Data: limit})
}
...
func (w *Worker) compactInode(res http.ResponseWriter, r *http.Request) {
	...
	cluster := r.FormValue(migcore.ClusterKey)
	volume := r.FormValue(migcore.VolNameKey)
	mpId := r.FormValue(migcore.MpIdKey)
	inodeId := r.FormValue(migcore.InodeIdKey)
	if len(volume) == 0 {
		migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeParamError, Msg: "volume name should not be an empty string"})
		return
	}
	var (
		mpIdNum int
	...
	if err = w.checkVolumeInfo(cluster, volume, data.Normal); err != nil {
		...
		migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeInternalError, Msg: msg})
		return
	}
	var (
	...
	if len(inodeExtents) <= 0 {
		migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeSuccess, Msg: "need not compact"})
		return
	}
	go func() {
		task := &proto.Task{
		...
		var migrateInode *migcore.MigInode
		migTask := NewMigTask(localIp, task, mc, volumeInfo)
		if err = migTask.SetMpInfo(); err != nil {
			log.LogErrorf("SetMpInfo cluster(%v) volName(%v) mpId(%v) inodeId(%v) err(%v)", cluster, volume, mpId, inodeId, err)
			return
		}
		if err = migTask.SetProfPort(); err != nil {
			log.LogErrorf("SetProfPort cluster(%v) volName(%v) mpId(%v) inodeId(%v) err(%v)", cluster, volume, mpId, inodeId, err)
			return
		}
		migrateInode, err = migcore.NewMigrateInode(migTask, inodeExtents[0])
		if err != nil {
	...
	migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeSuccess, Msg: "start exec inode compact task"})
}
...
func (w *Worker) migrationConfigAddOrUpdate(res http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeInternalError, Msg: err.Error()})
		return
	}
	if value, ok = w.masterClients.Load(migConfig.ClusterName); !ok {
		migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeInternalError, Msg: fmt.Sprintf("cluster(%v) not support", migConfig.ClusterName)})
		return
	}
...
func (w *Worker) migrationConfigList(res http.ResponseWriter, r *http.Request) {
	...
	migcore.SendReply(res, r, &proto.HTTPReply{Code: proto.ErrCodeSuccess, Msg: "success", Data: migrationConfig})
}
