...
struct ChunkMeta {
    ...
    Status<> Save(char* b) {
        return ChunkMetaBlocker::Save(block_size, chunk_meta_info, b, kChunkMetaName);
    }
    Status<> Load(const char* b) {
        return ChunkMetaBlocker::Load(block_size, b, &chunk_meta_info, kChunkMetaName);
    }
};
