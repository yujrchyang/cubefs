...
// Test implementation of BaseLogEntry
class TestLogEntry : public BaseLogEntry {
   public:
    TestLogEntry(LogRecordType type, const std::vector<uint8_t>& data, uint64_t id = 0)
        : type_(type), data_(data), id_(id == 0 ? GenerateID() : id) {}

    TestLogEntry(LogRecordType type, size_t size, uint64_t id = 0)
        : type_(type), data_(GenerateTestData(size)), id_(id == 0 ? GenerateID() : id) {}

    ~TestLogEntry() override = default;
    ...
    LogRecordType Type() override { return type_; }
...
SEASTAR_TEST_CASE(test_basic_append) {
    ...
    callbacks[LogRecordType::SliceMeta] =
        [](std::map<uint64_t, BaseLogEntryPtr>) -> seastar::future<Status<>> {
        co_return Status<>();
    };
    ...
    auto entry1 = seastar::make_shared<TestLogEntry>(LogRecordType::SliceMeta, 100);
    auto entry2 = seastar::make_shared<TestLogEntry>(LogRecordType::SliceMeta, 3000);
...
SEASTAR_TEST_CASE(test_throughput_benchmark) {
    ...
    // 预先创建所有 TestLogEntry 对象 (使用 shared_ptr)
    std::vector<seastar::shared_ptr<TestLogEntry>> entries;
    entries.reserve(kRecordCount);

    for (int i = 0; i < kRecordCount; i++) {
        auto type = LogRecordType::SliceMeta;
        entries.push_back(seastar::make_shared<TestLogEntry>(type, kRecordSize));
    }
...
SEASTAR_TEST_CASE(test_journal_replay) {
    ...
    for (size_t i = 0; i < kTestEntryCount; ++i) {
        ...
        auto entry = seastar::make_shared<TestLogEntry>(type, size, 1000 + i);
    ...
    auto replay_fn = [&](LogRecordType type, const char* data,
                         size_t size) -> seastar::future<Status<>> {
        Status<> s;

        // Unmarshal the test entry
        // TestLogEntry format: [id (8 bytes)][data (size-8 bytes)]
...
SEASTAR_TEST_CASE(test_checkpoint) {
    ...
    auto layout = JournalConfig{
        .start_offset = 0,
        .log_arena_size = 1ull << 20,  // Small arena (1MB) to trigger checkpoint faster
    };
    auto device = std::make_unique<MockDevice>();
    ...
    std::map<LogRecordType, CheckpointCallBack> callbacks;
    callbacks[LogRecordType::SliceMeta] =
        [&](std::map<uint64_t, BaseLogEntryPtr> entries) -> seastar::future<Status<>> {
        Status<> s;
    ...
    // Note: layout.log_arena_size is the size of a single arena (not total for both)
    const size_t entry_size = 100;                                    // Average entry size
    const size_t arena_size = layout.log_arena_size;                  // Single arena size
    const size_t arena_capacity_bytes = arena_size - kLogHeaderSize;  // Exclude header
    const size_t max_records = arena_capacity_bytes / kLogRecordSize;

    // Write enough entries to fill > 1 arena to trigger checkpoint
    // Assume ~30 entries can fit in one 4KB LogRecord (with 100 byte entries + overhead)
    ...
    std::cout << "  LogRecord size: " << kLogRecordSize << " bytes" << std::endl;
    ...
    for (size_t i = 0; i < target_entries; ++i) {
        auto type = LogRecordType::SliceMeta;
        auto entry = seastar::make_shared<TestLogEntry>(type, entry_size, 2000 + i);
    ...
    for (size_t i = 0; i < additional_entries; ++i) {
        auto type = LogRecordType::SliceMeta;
        auto entry =
            seastar::make_shared<TestLogEntry>(type, entry_size, 2000 + target_entries + i);
...
SEASTAR_TEST_CASE(test_arena_switch_checkpoint_required) {
    ...
    auto layout = JournalConfig{
        .start_offset = 0,
        .log_arena_size = 512ull << 10,  // Very small arena (512KB) to quickly fill
    };
    auto device = std::make_unique<MockDevice>();
    ...
    const size_t arena_capacity = (layout.log_arena_size - kLogHeaderSize) / kLogRecordSize;
    const size_t entries_per_record = 30;
    ...
    for (size_t i = 0; i < entries_to_fill_arena * 2; ++i) {
        auto type = LogRecordType::SliceMeta;
        auto entry = seastar::make_shared<TestLogEntry>(type, entry_size, 10000 + i);
    ...
    for (size_t i = 0; i < entries_to_fill_arena * 2; ++i) {
        auto type = LogRecordType::SliceMeta;
        auto entry = seastar::make_shared<TestLogEntry>(type, entry_size, 20000 + i);
    ...
    for (size_t i = 0; i < 10; ++i) {
        auto type = LogRecordType::SliceMeta;
        auto entry = seastar::make_shared<TestLogEntry>(type, entry_size, 30000 + i);
...
SEASTAR_TEST_CASE(test_crash_recovery) {
    ...
    auto layout = JournalConfig{
        .start_offset = 0,
        .log_arena_size = 2ull << 20,  // 2MB arena for faster testing
    };
    ...
    for (size_t cycle = 0; cycle < kNumCycles; ++cycle) {
        ...
        for (size_t i = 0; i < kEntriesPerCycle; ++i) {
            ...
            auto entry = seastar::make_shared<TestLogEntry>(type, data, id);
...
SEASTAR_TEST_CASE(test_disk_failure) {
    ...
    for (size_t i = 0; i < 50; ++i) {
        auto type = LogRecordType::SliceMeta;
        auto entry = seastar::make_shared<TestLogEntry>(type, entry_size, i);
    ...
    for (size_t i = 0; i < 10; ++i) {
        auto type = LogRecordType::SliceMeta;
        auto entry = seastar::make_shared<TestLogEntry>(type, entry_size, 300 + i);
