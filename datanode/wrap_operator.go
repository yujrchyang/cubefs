...
func (s *DataNode) handleExtentRepairReadPacket(p *repl.Packet, connect net.Conn, isRepairRead bool) {
	...
	for {
		...
		err = func() error {
			...
			// 变更内容1：对OpStreamRead，强制检查pending，此外，对于严格模式下的其他opcode(OpStreamFollowerRead, OpExtentRepairRead)是否需要做pending检查，需验证
			// 变更内容2：对OpStreamFollowerRead，由于一致性读场景下使用该op，需要读到最新数据，因此也需要进行检查
