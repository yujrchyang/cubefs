...
import (
	...
	dataSdk "github.com/cubefs/cubefs/sdk/data"
...
func getDataSdkWrapper(volume string, masters []string) (w *dataSdk.Wrapper, err error) {
	...
	limit := MaxRetryLimit
retry:
	readAheadMemMB, readAheadWindowMB := int64(0), int64(0)
	w, err = dataSdk.NewDataPartitionWrapper(volume, masters, dataSdk.Normal, readAheadMemMB, readAheadWindowMB, nil)
	if err != nil {
		if limit <= 0 {
