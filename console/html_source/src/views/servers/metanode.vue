<template>
  <div class="alarm">
    <div class="fr mb20">
      ...
      <el-button type="primary" class="ml5" @click="queryList">{{
        $t("chubaoFS.tools.Search")
      }}</el-button>
    </div>
    <div class="data-block" v-loading="resData.loading">
      <el-table :data="resData.resLists" class="mt20" style="width: 100%">
        <!--<el-table-column type="index" label="#"></el-table-column>-->
        <el-table-column prop="address" :label="$t('chubaoFS.servers.Address')">
          <template slot-scope="scope">
            <div class="volume-name" @click="goServerDetail(scope.row)">
              {{ scope.row.addr }}
            </div>
          </template>
        </el-table-column>
        <el-table-column
          prop="toMetaNode.zoneName"
          :label="$t('chubaoFS.servers.Zone')"
        >
          <!--<template slot-scope="scope">-->
        ...
        <el-table-column
          prop="iD"
          :label="$t('chubaoFS.servers.identity')"
        ></el-table-column>
        <el-table-column
          prop="toMetaNode.metaPartitionCount"
          :label="$t('chubaoFS.servers.PartitionCount')"
          width="150"
        ></el-table-column>
        <el-table-column
          prop="toMetaNode.used"
          :label="$t('chubaoFS.servers.Used')"
        ></el-table-column>
        <el-table-column
          prop="toMetaNode.total"
          :label="$t('chubaoFS.servers.Total')"
        ></el-table-column>
        <el-table-column
          prop="toMetaNode.available"
          :label="$t('chubaoFS.servers.Available')"
        ></el-table-column>
        <el-table-column
          prop="toMetaNode.ratio"
          :label="$t('chubaoFS.servers.UsedRate')"
        ></el-table-column>
        <el-table-column
          prop="toMetaNode.reportTime"
          :label="$t('chubaoFS.servers.ReportTime')"
          width="150"
        ></el-table-column>
        <el-table-column
          prop="toMetaNode.isActive"
          :label="$t('chubaoFS.servers.IsActive')"
        >
          <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      ...
      this.resData.resLists = this.resetData(
        this.storageLists.slice(start, end)
      );
    },
    resetData(data) {
      data.forEach(eachItem => {
        eachItem.toMetaNode.available = formatSize(
          eachItem.toMetaNode.total - eachItem.toMetaNode.used,
          10
        );
        ...
        eachItem.toMetaNode.ratio = parseFloat(
          (eachItem.toMetaNode.ratio * 100).toFixed(2)
        );
        const reportTime = eachItem.toMetaNode.reportTime;
        eachItem.toMetaNode.reportTime = reportTime
          .slice(0, reportTime.indexOf("."))
          .replace("T", " ");
      });
      return data;
    },
    queryList() {
      ...
      this.apollo
        .query(this.url.cluster, baseGql.queryMetaList, variables)
        .then(res => {
