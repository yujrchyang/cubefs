<template>
  <div class="cluster volume">
    <el-form :inline="true" :model="query">
      ...
      <el-form-item label="卷名：" required>
        <el-select
    ...
    <div class="charts nodata" v-show="chartNoData">
      暂无数据
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    queryChart() {
      ...
      if (!this.query.volume || this.query.volume == "") {
        this.$message.error("请选择卷名！");
        return;
      }
      ...
      this.apollo
        .query(this.url.consoleTraffic, baseGql.volDetails, params)
        .then(res => {
          if (res.data) {
            this.buildChart(res.data.volDetails.data);
          } else {
            this.$Message.error(res.message);
          }
        })
