...
func (m *Server) setNodeInfoHandler(w http.ResponseWriter, r *http.Request) {
	...
	if val, ok := params[proto.RateLimitKey]; ok {
		...
		if zone == "" && vol == "" {
			err = errors.NewErrorf("zone, vol can't be empty at the same time")
			sendErrReply(w, r, newErrHTTPReply(err))
			return
		}
		if op == 0 {
			err = errors.NewErrorf("op is empty")
			sendErrReply(w, r, newErrHTTPReply(err))
			return
		}
		rateLimit := val.(uint64)
		val, ok := params[proto.RateLimitIndexKey]
		if !ok {
			err = errors.NewErrorf("rateLimitIndex is empty")
			sendErrReply(w, r, newErrHTTPReply(err))
			return
		}
		rateLimitIndex := val.(uint64)
		if rateLimitIndex >= multirate.IndexMax {
			err = errors.NewErrorf("rateLimitIndex should be less than %v", multirate.IndexMax)
			sendErrReply(w, r, newErrHTTPReply(err))
			return
		}
...
func parseBoolFieldToUpdateVol(r *http.Request, vol *Vol) (followerRead, nearRead, authenticate, enableToken, autoRepair,
	...
	if notCacheNodeStr := r.FormValue(proto.NotCacheNodeKey); notCacheNodeStr != "" {
		if notCacheNode, err = strconv.ParseBool(notCacheNodeStr); err != nil {
			err = unmatchedKey(proto.NotCacheNodeKey)
			return
		}
	} else {
		notCacheNode = vol.notCacheNode
...
func parseAndExtractSetNodeInfoParams(r *http.Request) (params map[string]interface{}, err error) {
	...
	if val := r.FormValue(moduleKey); val != "" {
		params[moduleKey] = val
	}
