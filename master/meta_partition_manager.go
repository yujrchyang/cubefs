...
func (c *Cluster) checkMetaPartitionRecoveryProgress() {
	...
	c.BadMetaPartitionIds.Range(func(key, value interface{}) bool {
		...
		if vol.Status == proto.VolStMarkDelete || (int(vol.mpReplicaNum) <= normalReplicaCount && int(partition.RecorderNum) <= len(partition.Recorders) && partition.allReplicaHasRecovered()) {
			partition.RLock()
...
func (c *Cluster) fulfillMetaReplica(partitionID uint64, badAddr string) (isPushBackToBadIDs bool) {
	...
	if err = c.addMetaReplica(partition, newPeer.Addr, vol.DefaultStoreMode, vol.PersistenceMode, vol.BoundBucket); err != nil {
		return
	}
