...
func (f *Node) Open(ctx context.Context, req *fuse.OpenRequest, resp *fuse.OpenResponse) (handle fs.Handle, err error) {
	...
	if log.IsDebugEnabled() {
		log.LogDebugf("TRACE Open: ctx(%v) ino(%v) req(%v) resp(%v) (%v)", proto.GetContextReq(ctx), ino, req, resp, time.Since(start))
	}
	return f, nil
}
...
func (f *Node) Release(ctx context.Context, req *fuse.ReleaseRequest) (err error) {
	...
	ino := f.inode
	if log.IsDebugEnabled() {
		log.LogDebugf("TRACE Release enter: ctx(%v) ino(%v) req(%v)", proto.GetContextReq(ctx), ino, req)
	}
	...
	if err != nil {
		log.LogErrorf("Release: close writer failed, ctx(%v), ino(%v) req(%v) err(%v)", proto.GetContextReq(ctx), ino, req, err)
		return fuse.EIO
	}
	...
	if log.IsDebugEnabled() {
		log.LogDebugf("TRACE Release: ctx(%v) ino(%v) req(%v) (%v)", proto.GetContextReq(ctx), ino, req, time.Since(start))
	}
	return nil
}
...
func (f *Node) Read(ctx context.Context, req *fuse.ReadRequest, resp *fuse.ReadResponse) (err error) {
	...
	if log.IsDebugEnabled() {
		log.LogDebugf("TRACE Read enter: ctx(%v) ino(%v) offset(%v) reqsize(%v) req(%v)", proto.GetContextReq(ctx), f.inode, req.Offset, req.Size, req)
	}
	...
	if err != nil && err != io.EOF {
		msg := fmt.Sprintf("Read: ctx(%v) ino(%v) req(%v) err(%v) size(%v)", proto.GetContextReq(ctx), f.inode, req, err, size)
	...
	if size > 0 {
		...
		log.LogWarnf("Read: ctx(%v) ino(%v) offset(%v) reqsize(%v) req(%v) size(%v)", proto.GetContextReq(ctx), f.inode, req.Offset, req.Size, req, size)
	}

	if log.IsDebugEnabled() {
		log.LogDebugf("TRACE Read: ctx(%v) ino(%v) offset(%v) reqsize(%v) req(%v) size(%v) (%v)", proto.GetContextReq(ctx), f.inode, req.Offset, req.Size, req, size, time.Since(start))
	}
	return nil
}
...
func (f *Node) Write(ctx context.Context, req *fuse.WriteRequest, resp *fuse.WriteResponse) (err error) {
	...
	log.LogDebugf("TRACE Write enter: ctx(%v) ino(%v) offset(%v) len(%v) filesize(%v) flags(%v) fileflags(%v) req(%v)", proto.GetContextReq(ctx), ino, req.Offset, reqlen, filesize, req.Flags, req.FileFlags, req)
	...
	if err != nil {
		msg := fmt.Sprintf("Write: ctx(%v) ino(%v) offset(%v) len(%v) err(%v)", proto.GetContextReq(ctx), ino, req.Offset, reqlen, err)
	...
	resp.Size = size
	if size != reqlen {
		log.LogErrorf("Write: ctx(%v) ino(%v) offset(%v) len(%v) size(%v)", proto.GetContextReq(ctx), ino, req.Offset, reqlen, size)
	}

	if waitForFlush {
		if err = Sup.ec.Flush(ctx, ino); err != nil {
			msg := fmt.Sprintf("Write: flush failed, ctx(%v) ino(%v) offset(%v) len(%v) err(%v) req(%v)", proto.GetContextReq(ctx), ino, req.Offset, reqlen, err, req)
	...
	log.LogDebugf("TRACE Write: ctx(%v) ino(%v) offset(%v) len(%v) flags(%v) fileflags(%v) req(%v) (%v)",
		proto.GetContextReq(ctx), ino, req.Offset, reqlen, req.Flags, req.FileFlags, req, time.Since(start))
	return nil
}
...
func (f *Node) Fsync(ctx context.Context, req *fuse.FsyncRequest) (err error) {
	...
	log.LogDebugf("TRACE Fsync enter: ctx(%v) ino(%v)", proto.GetContextReq(ctx), f.inode)
	start := time.Now()
	...
	log.LogDebugf("TRACE Fsync: ctx(%v) ino(%v) (%v)", proto.GetContextReq(ctx), f.inode, time.Since(start))
	return nil
}
