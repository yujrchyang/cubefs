...
func NewSuper(opt *proto.MountOptions, first_start bool, metaState *meta.MetaState,
	...
	var extentConfig = &data.ExtentConfig{
		...
		OnGetExtentsNoModifyAccessTime: s.mw.GetExtentsNoModifyAccessTime,
...
func (s *Super) BatchDownload(w http.ResponseWriter, r *http.Request) {
	...
	respWriter := &data.BatchDownloadRespWriter{ResChan: make(chan *data.DownloadResult, len(batchInfos))}
	...
	for dRes := range respWriter.ResChan {
		if isRespErr {
			data.PutBlockBuf(dRes.RespData)
			continue
		}
...
func (s *Super) BatchDownloadPath(w http.ResponseWriter, r *http.Request) {
	...
	respWriter := &data.BatchDownloadRespWriter{ResChan: make(chan *data.DownloadResult, pathCount)}
	for _, batch := range batchArr {
