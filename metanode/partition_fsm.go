...
func (mp *metaPartition) Apply(command []byte, index uint64) (resp interface{}, err error) {
	...
	defer func() {
		mp.cancelWaitApplyCompleteCtx(index)
		if err != nil {
	...
	case opFSMFileMigExtentMerge:
		...
		resp, err = mp.fsmFileMigExtentsMerge(dbWriteHandle, im, msg.ReqInfo)
...
func (mp *metaPartition) cancelWaitApplyCompleteCtx(index uint64) {
	mp.waitCtxMu.RLock()
	waitCtx := mp.waitCtx
	...
	mp.waitCtxMu.Lock()
	if mp.waitCtx != nil && index >= mp.lastValidIndexOnLeaderChange {
		if mp.waitCancelFunc != nil {
			mp.waitCancelFunc()
			mp.waitCancelFunc = nil
		}
		mp.waitCtx = nil
	}
	mp.waitCtxMu.Unlock()
	log.LogDebugf("meta partition apply pending action complete, partitionID: %v, curApplyIndex: %v, lastIndexOnLeaderChange: %v",
...
func (mp *metaPartition) Snapshot(recoverNode uint64, isRecorder bool) (snap raftproto.Snapshot, err error) {
	...
	snapV = GetMatchedSnapV(version)
	log.LogInfof("Snapshot: mp[%v] get recover node[%v] version[%s], snapshotV[%v]", mp.config.PartitionId, recoverNode, version.VersionStr(), snapV)
	if snapV == BaseSnapshotV {
